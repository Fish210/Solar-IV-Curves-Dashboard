...
export const TOUR_STEPS = [
  { kicker: "00 / ORIENTATION", title: "Welcome to Solar I-V Lab", desc: "A university-grade workbench for photovoltaic I-V characterization. The next minute walks you through every panel — uploads, sweeps, parameter extraction, and the lab assistant.", nav: null, vizT: null, chat: false, pos: "center", labels: [] },
  ...
  { kicker: "02 / I-V CURVES", title: "Current vs. Voltage", desc: "Interactive sweep plot. Toggle channels above the chart, scrub the brush to zoom the knee region, and use voltage lookup for an exact crosshair readout.", nav: "viz", vizT: "iv", chat: false, pos: "bottom-right", labels: [{ text: "Channel toggles", anchor: "channels", side: "top" }, { text: "Voltage cursor lookup", anchor: "vlookup", side: "right" }, { text: "Brush to zoom the knee", anchor: "iv-chart", side: "left" }] },
  { kicker: "03 / P-V CURVES", title: "Power vs. Voltage", desc: "Power profile with shaded area fills. Solid dots mark each channel's maximum power point — the operating sweet spot for that condition.", nav: "viz", vizT: "pv", chat: false, pos: "bottom-right", labels: [{ text: "P_max markers + area fill", anchor: "pv-chart", side: "left" }] },
  ...
  { kicker: "08 / READY", title: "You're calibrated.", desc: "Explore the sample dataset or import your own. Named profiles get persistent local storage and a saved library — demo mode works in-session only.", nav: "home", vizT: null, chat: false, pos: "center", labels: [] },
];
        ...
        <div className="corners" style={{ background: "linear-gradient(180deg," + t.card + "," + t.cardAlt + ")", borderRadius: 12, border: "1px solid " + t.border, padding: "22px 22px 18px", boxShadow: "0 24px 60px rgba(0,0,0,.5),0 0 0 1px " + t.accent + "22", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", gap: 6, justifyContent: "space-between", alignItems: "center", paddingTop: 14, borderTop: "1px dashed " + t.border }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {p.step > 0 && <button onClick={p.onBack} className="lift" style={{ padding: "7px 13px", borderRadius: 7, border: "1px solid " + t.border, background: t.card, color: t.textM, fontSize: 10.5, fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>← Back</button>}
              <button onClick={() => { if (p.step < total - 1) p.onNext(); else p.onSkip(); }} className="lift" style={{ padding: "7px 16px", borderRadius: 7, border: "none", background: "linear-gradient(135deg," + t.accent + "," + t.accent2 + ")", color: "#fff", fontSize: 10.5, fontWeight: 600, display: "flex", alignItems: "center", gap: 5, boxShadow: "0 4px 14px " + t.accentG }}>{p.step < total - 1 ? "Next →" : "✓ Finish"}</button>
            </div>
          </div>
        </div>
      </div>
